...
% earlierback=back;
back=difference<0.03;%thresholdinitial;
% back=~back;
...
figure;
% plot3([90:110],mean_shape(90:110,1),mean_shape(90:110,2),'r','LineWidth',4)
plot3([1:size(mean_shape,1)],mean_shape(:,1),mean_shape(:,2),'r','LineWidth',4);
...
x=[1:size(mean_shape,1)]';
y=mean_shape(:,1);
sm_mean_shape(:,1)= smooth(x,y,0.5,'loess');
y=mean_shape(:,2);
sm_mean_shape(:,2)= smooth(x,y,0.5,'loess');
...
for l=1:size(validTraj,2)
    ...
    if l~=1
    sm_mean_shape(start+1,:)=mean_shape(start+1,:);
    sm_mean_shape(start+diffe,:)=mean_shape(start+diffe,:);
    end
for i=1:size(TrajCoord{l},3)
Trajectory = TrajCoord{l}(:,:,i);
[Y c scale]=pre_shape(Trajectory);
diffmat = repmat(difference(l,:),size(ind,2),1);
% A=Y'*(mean_shape(ind,:)-diffmat);
%   [L, D, M] = svd(A);
% T = M * L';
pp = (mean_shape(ind,:)-diffmat)* T{l}(:,:,i);
p = (sm_mean_shape(ind,:)-diffmat)* T{l}(:,:,i);%tran(:,:,i);%+tr.c;
d(i)= norm(p-(sm_mean_shape(ind,:)-diffmat));
% come back from preshape
  kkk= p*scale+repmat(c,size(p,1),1);
  ppp(:,:,i)= pp*scale+repmat(c,size(p,1),1);
  StabOTDM{l}(:,:,i)=kkk;
%   plot3(ind,Trajectory(:,1),Trajectory(:,2),'r','LineWidth',1);
%   hold on
%   plot3(ind,kkk(:,1),kkk(:,2),'b','LineWidth',1);
%   
