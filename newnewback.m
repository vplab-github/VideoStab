...
addpath(genpath('./'));
clear all;f='A';set = 'A';
% InputVideoName1='1.avi';
folder = ['./data/' set '/' f '/'];
fileLocation = [folder set '_' f 'Tracks573.dat'];
% fileLocation = [folder f 'Tracks573.dat'];
...
% if d<0.01 & nnz(trajidsback==i)==0
    Trajectoryinframe(i,ind)=1;
     mean_shape*size(OTDM,3)-Ypretran+(Ypre(:,:,i)*tran(:,:,i));
    rot = mean_shape*size(OTDM,1);
...
% plot3([1:size(Y,1)],kkk(:,1),kkk(:,2),'LineWidth',1); hold on
% pause(0.5); 
% hold on
% end
% close all;
